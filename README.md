...
Streaming formatters — CSV (formula-injection safe), JSONL (chain-verifiable
envelope), JSON, HTML — plus a dompdf-backed `PdfRenderer` and a
...
`suggest`ed, never `require`d — the bridge is a clean no-op when it is absent.
