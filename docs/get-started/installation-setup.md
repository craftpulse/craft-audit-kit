...
Audit Kit ships no migrations today, so this call is currently a no-op. Wire it anyway. It is the seam that lets a future kit migration reach every install without a coordinated release across every consuming plugin.
